...
use Voronoi\Exception\VoronoiException;

/**
 * Class for creating and manipulating Voronoi diagrams using Fortune's algorithm.
 *
 * This was originally inspired by the JavaScript library created by Raymond Hill. Most
 * of this work was done by Samuel Roze.
 *
...
class Voronoi
{
    ...
    private array $beach_section_junkyard = [];
    private array $circle_event_junkyard = [];
    protected RBTree $beach_line;

    protected RBTree $circle_events;
    private $first_circle_event;

    public function __construct()
    {
        $this->beach_line = new RBTree();
        $this->circle_events = new RBTree();
    ...
    public function compute($sites, $bbox)
    {
        ...
        for (; ;) {
            ...
            $circle = $this->first_circle_event;
        ...
        return [
            ...
            'cells' => $this->cells,
            'edges' => $this->edges,
            'execTime' => (microtime(true) - $startTime) / 1000, // in seconds
        ];
    }
    ...
    // rhill 2011-06-02: A lot of beach section instantiations
    // occur during the computation of the Voronoi diagram;
    // somewhere between the number of sites and twice the
    // number of sites, while the number of beach sections on the
    // beach line at any given time is comparatively low. For this
    // reason, we reuse already created beach sections, in order
    // to avoid new memory allocation. This resulted in a measurable
    // performance gain.
    public function createBeachSection($site): BeachSection
    {
        $beachSection = array_pop($this->beach_section_junkyard);
        if ($beachSection) {
    ...
    public function detachBeachSection($beachSection)
    {
        ...
        $this->beach_line->removeNode($beachSection); // remove from RB-tree
        $this->beach_section_junkyard[] = $beachSection; // mark for reuse
    }
    ...
    public function addBeachSection($site)
    {
        ...
        $this->beach_line->insertSuccessor($lArc, $newArc);
        ...
        if ($lArc === $rArc) {
            ...
            $this->beach_line->insertSuccessor($newArc, $rArc);
        ...
        // on the beach line -- this can happen *only* if *all* the previous beach
        // sections currently on the beach line share the same y value as
    ...
    public function attachCircleEvent($arc)
    {
        ...
        // If site of left beach section is same as site of
        // right beach section, there can't be convergence
        if ($lSite === $rSite) {
            return;
        }

        // Find the circumscribed circle for the three sites associated
        // with the beach section triplet.
        ...
        $circleEvent = array_pop($this->circle_event_junkyard);
        if (!$circleEvent) {
        ...
        $this->circle_events->insertSuccessor($predecessor, $circleEvent);
        if (!$predecessor) {
            $this->first_circle_event = $circleEvent;
        }
    }
    ...
    public function detachCircleEvent($arc)
    {
        ...
        if ($circle) {
            if (!$circle->previous) {
                $this->first_circle_event = $circle->next;
            }

            $this->circle_events->removeNode($circle); // remove from RB-tree
            $this->circle_event_junkyard[] = $circle;
    ...
    public function equalWithEpsilon($a, $b)
    {
        return abs($a - $b) < self::EPSILON;
    }

    public function greaterThanWithEpsilon($a, $b)
    {
        return $a - $b > self::EPSILON;
    }

    public function greaterThanOrEqualWithEpsilon($a, $b)
    {
        return $b - $a < self::EPSILON;
    }

    public function lessThanWithEpsilon($a, $b)
    {
        return $b - $a > self::EPSILON;
    }

    public function lessThanOrEqualWithEpsilon($a, $b)
    {
        return $a - $b < self::EPSILON;
    }
}
